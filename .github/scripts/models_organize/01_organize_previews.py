...
默认（不传 <目录>）扫描 Models 与 Other-YSM-Models；传任意目录则把该目录作为根，自动递归
检查其下需要整理预览图的模型目录（Blockbench-Models 等其余根目录也可显式传入）。
"""
...
# 默认扫描根目录（无路径参数时）：Models + Other-YSM-Models。
# Blockbench-Models 等其余根目录可在路径参数中显式传入。
DEFAULT_ROOTS = [
...
def main() -> int:
    ...
    if args.apply and total_ops > 0 and args.with_gen_readmes:
        ...
        if rc != 0:
            ...
            return rc
    elif args.apply and total_ops > 0:
