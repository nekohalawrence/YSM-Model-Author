...
数据（外置，可手工维护，位于 .github/data/）：
  author-info/authors.json         作者集中数据（name 数组 / role / platforms），author_index.py --data 生成
  author-info/platform_map.json    平台分类映射 {分类: [平台键...]}（分类为键、平台键列表为值）
  author-info/models_meta.json     模型 -> co-creator 作者列表（按需生成，无记录时文件不存在）
