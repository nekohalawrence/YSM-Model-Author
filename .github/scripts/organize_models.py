...
用法:
  python .github/scripts/organize_models.py <文件或目录>... [选项]
...
def update_root_readme(root: Path) -> None:
    script = root / '.github' / 'scripts' / 'build_readme_authors.py'
...
def build_authors_index(root: Path) -> None:
    ...
    script = root / '.github' / 'scripts' / 'build_authors_index.py'
    if not script.is_file():
...
def run_rename_model_folders(root: Path) -> None:
    script = root / '.github' / 'scripts' / 'rename_model_folders.py'
    if not script.is_file():
...
def run_generate_model_readmes(root: Path) -> None:
    script = root / '.github' / 'scripts' / 'generate_model_readmes.py'
    if not script.is_file():
