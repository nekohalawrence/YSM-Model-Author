...
# -*- coding: utf-8 -*-
"""
作者索引工具：生成集中作者数据（authors.json）+ 重建根 README 作者表。

由原 build_authors_index.py（数据生产者）与 build_readme_authors.py（根表渲染）
合并而来：作者索引的"数据"与"呈现"统一在此，供 cli / pipeline / organize / audit 调用。

用法：
  python author_index.py                  # --data + --readme 全做
  python author_index.py --data           # 只生成 authors.json
  python author_index.py --readme         # 只更新根 README / README-EN 作者表
  python author_index.py --data --check   # 只检查 authors.json 差异（不写盘）
"""
import argparse
import re
import sys
