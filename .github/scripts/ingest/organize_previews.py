...
用法:
    python .github/scripts/ingest/organize_previews.py                      # 预览将移动哪些文件
    python .github/scripts/ingest/organize_previews.py --rename             # 预览将如何重命名预览图
    python .github/scripts/ingest/organize_previews.py --apply              # 真正移动 + 重生成 README
    python .github/scripts/ingest/organize_previews.py --apply --rename     # 真正重命名 + 重生成 README
    python .github/scripts/ingest/organize_previews.py --apply --no-regenerate
    python .github/scripts/ingest/organize_previews.py --root Models        # 只处理指定根目录
    python .github/scripts/ingest/organize_previews.py --only "Models/0001/AveMujica_三角初华_LB"
"""
...
def main() -> int:
    ...
    if args.apply and total_ops > 0 and not args.no_regenerate:
        ...
        print('提示:已跳过 README 重生成(--no-regenerate)。请手动运行 '
              'python .github/scripts/publish/generate_model_readmes.py 更新引用。')
