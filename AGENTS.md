...
## 参数命名规范（所有带 CLI 参数的脚本）

> 详细版见 `.github/scripts/README.md`。新增/修改脚本参数必须遵守；旧脚本渐进迁移，不一次性大改。

- **写盘标志**：一律 `--apply`（真正执行，默认 dry-run）；不用 `--dry-run` 这类反义写法。
- **通用参数**（跨脚本同名同义）：`--apply`（写盘）、`--root PATH`（仓库根）、`--kb DIR`（知识库目录）、`--only PATH`（限定路径）、`--verbose`（详细输出）。
- **动作类**：一律「动词-对象」kebab-case（`--rename-files`、`--merge-authors`、`--build-category-map`）；`--with-*` 仅表示"联动后续步骤"。
- **报告 vs 展示**：`--report-*` = 生成报告；`--show-*` = 查看结果，二者不混用。
- **形式**：统一 kebab-case（连字符），不用下划线/驼峰；取值用 `--flag VALUE`，必填主输入用位置参数；避免同一名字既是取值参数又是前缀（如 `--report` 与 `--report-*` 冲突）。
